...
Поэтому prompt-visible инструкции разрешают экспортированные shim-ы по
проектному и глобальному scope и никогда не адресуют `runtime_root`
относительно `SKILL.md`. Это правило сохраняется, когда адаптер зеркалирует
контекст копированием, а не symlink-ом.
