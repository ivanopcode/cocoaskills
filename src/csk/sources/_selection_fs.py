...
# Names of the ``os`` functions the confined descent calls with ``dir_fd=``
# whose membership in ``os.supports_dir_fd`` gates selection. ``os.lstat``
# is deliberately not a member of this set: on macOS with Python 3.11/3.12
# ``os.lstat`` accepts ``dir_fd=`` at runtime but is absent from
# ``os.supports_dir_fd``, so requiring membership would refuse a healthy
# POSIX lane. The descent still attempts the ``dir_fd=`` form first and
# every ``lstat``-derived identity is re-verified against the frozen
# Phase-A binding and the opened descriptor.
_REQUIRED_DIR_FD_FUNCTIONS: Final = ("open", "stat", "readlink")


# Import-time identities of the gated functions. The predicate compares
# these against the live ``os.supports_dir_fd`` set instead of reading the
# current ``os`` attributes: fault injection and tracing legitimately wrap
# ``os.open``/``os.stat``/``os.readlink`` in-process (including this
# repository's own observation harness), and a wrapper must never change
# the capability answer. The set itself and ``O_DIRECTORY`` are always
# read live.
_DIR_FD_ORIGINALS: Final = {
    name: getattr(os, name, None) for name in _REQUIRED_DIR_FD_FUNCTIONS
}


# The single skip reason every descriptor-traversal test uses on a runtime
...
def supports_descriptor_traversal() -> bool:
    ...
    for name in _REQUIRED_DIR_FD_FUNCTIONS:
        original = _DIR_FD_ORIGINALS[name]
        if original is None or original not in supported:
            return False
    return True
...
def _missing_traversal_mechanisms() -> list[str]:
    ...
    for name in _REQUIRED_DIR_FD_FUNCTIONS:
        original = _DIR_FD_ORIGINALS[name]
        if supported is None or original is None or original not in supported:
            missing.append(f"dir_fd support for os.{name}")
    return missing
    ...
    def open(
        ...
        *,
        ...
        if not supports_descriptor_traversal():
            missing = _missing_traversal_mechanisms()
            # A real no-capability runtime always names at least one missing
            # mechanism; the fallback covers only an injected False on a
            # capable host, so the refusal still names the mechanism class.
            unavailable = (
                ", ".join(missing)
                if missing
