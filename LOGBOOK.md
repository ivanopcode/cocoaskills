...
(POSIX-only, mechanisms named, `__cause__` None) before Phase-A preflight where the runtime lacks
descriptor-relative traversal, decided from `os.supports_dir_fd` + `O_DIRECTORY` by one named predicate
the tests share; traversal tests skip with one named reason and refusal/v1 positives run everywhere.
Two measurements shaped the predicate: `os.lstat` accepts `dir_fd=` on macOS 3.11/3.12 while missing
from `os.supports_dir_fd`, so it is excluded from the required set (a require-lstat mutant dies on the
py311-shaped row); and membership is tested against import-time function identities because live
`getattr(os, name)` flips under this repo's own fault-injection/tracing wrappers (first candidate turned
44 boundary tests red). Full ordinary suite green serially in four chunks (7093 passed, 122 skipped,
exit 0 each); two xdist-at-scale sibling-oracle sensitivities (file-redirect observation pollution, one
load-dependent did-not-raise cell unproducible by this diff) reported in the results, owned elsewhere.
Linux/Windows proved only by hosted lanes; Windows must be green via declared skips plus positives.
...
## 2026-09-17 TASK-260916-2wjh3m re-entry: selection semantics on the sibling seam

Re-entered after the recorded fallback split the filesystem access layer into
accepted TASK-260917-3q5h87. This leaf consumes that seam only
(`SelectionSession` open/descend/child-directories/snapshot/managed-boundary/
reverify plus the `read_regular_path` legacy helpers); `selection.py` reaches
no filesystem primitive of its own (static one-seam test) and dropped its
unused `os`/`stat` imports. Closed the remaining AC (f) clause: directly
declared skill requirements naming one dependency with different canonical
repository identities now fail `source_name_conflict` within a collection and
across the whole set, mirroring the source half of `closure._unify`
(identical declarations unify; local paths, malformed sources, and ref-vs-ref
comparison defer to closure validation at publication). Proven by 7 new
conflict-matrix tests (2 failed before the fix) and 7 regenerated narrowing
mutants, all killed. Moved the 46 boundary-mechanics tests intact to
`tests/test_selection_boundary_property.py` under sibling ownership (verified
byte-identical to HEAD); this file keeps semantics, mapping representatives,
and the frontmatter language. Full evidence in `TASK-260916-2wjh3m_results.md`.

## 2026-09-17 TASK-260916-2wjh3m refresh and scope fix

Ran `worktree refresh-candidate` onto trunk `749dd9f` (branch `e5b935f`),
resolving the three append/append replay conflicts keep-both via explicit
`{checkpoint_oid, path, sha256, content}` resolutions, then restored the four
trunk files the refresh left out of the working tree and merged the `1iyslr`
notes back into `.spec` and `LOGBOOK`. Carried the sibling scope correction:
the one-seam static test now walks the selection import closure
(`selection`, `_selection_fs`, `errors`, `skillfile_v2`) instead of the
package directory, so trunk's `repository_policy.py` reader is out of scope.
Re-ran all narrow gates green, all 7 narrowing mutants killed on this tree,
and the full ordinary suite green twice (7115 passed, 172 skipped). Found and
reported (not fixed; sibling-owned) an fd-offset oracle sensitivity: with
stderr redirected to a regular file under xdist, the worker observes the
controller's own log writes as `stream.read` drift. Full evidence in
`TASK-260916-2wjh3m_results.md`.

## 2026-09-17 TASK-260916-2wjh3m revision 25 (round 2 of 3)

Closed the revision-24 F1 bypass in the import-closure scope walk (sibling
instrument, correction still riding here): the walk now fails closed —
dynamic code-loading facilities are scope violations, bare package binds are
refused as unresolvable scope and widen the scan to the whole package,
level-2 `..sources` resolves, subpackage `__init__` files are descended
into, and the package `__init__` is seeded. Committed the six miss shapes
as a 39-case synthetic table plus a real-tree pin and a 5-family shadow
end-to-end against the real shipped test, with two narrowing mutants
(scope half and detection half, each with a named killer and controls).
Folded the reviewer's conflict probes into the suite as parametrised
families (manifest kinds, 3-chain, 21 hostile git strings, 11 malformed
shapes, dual-manifest, deep-nest, duplicate keys, each across entries).
Production `selection.py` is byte-identical to revision 24 (sha256
`55410b81…a17e5`); the four review notes (N1 managed-spelled names, N2
empty selector list, N4 undecodable-name platform bound) are recorded in
the results, not coded around. Full ordinary suite green on the final tree
(7240 passed, 172 skipped). Full evidence in
`TASK-260916-2wjh3m_results.md`.

## 2026-09-17 TASK-260916-2wjh3m revision 26 (round 3 of 3)

Closed the revision-25 F1 class (sibling instrument, correction still
riding here) by observation instead of enumeration: a fresh subprocess
imports the selection entry point, drives both public entry points over
a one-member fixture, and reports `sys.modules` plus audit `exec` and
`open` filenames; every file-backed signal must be in the static import
closure. The `exec` signal proved necessary: `runpy.run_module` executes
its target as `__main__` and never enters `sys.modules` (verified
empirically), so a `sys.modules`-only snapshot misses exactly the F1
shape. The static dynamic-facility enumeration stays as defence in depth
and its incompleteness is now a note, not a bypass. Committed the whole
miss corpus: five statically silent shadow families (runpy from-import,
alias and run-path, getattr-importlib, package star-all) that fire only
via the runtime half, a silent-shape synthetic table pinning the static
contract, and a narrowing mutant dropping exactly `evil_shadow.py` from
the runtime comparison (3 shadow killers fail, 4 controls pass).
M-CLOSURE now shows the backstop working: its shadow control passes
under the mutant because the runtime check fires where the static walk
misses. Production `selection.py` is byte-identical to revision 24
