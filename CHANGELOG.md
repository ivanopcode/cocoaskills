...
- Схема манифеста 8. Команда `go-v1` объявляет корни первопартийных модулей в поле `modules`: каталоги модулей Go того же пакета, которые корень сборки заменяет через `replace`. csk сверяет объявление с материализованным набором директив из `<build_root>/vendor/modules.txt` один к одному, никогда не читая замену как инструкцию, и распространяет скан на cgo, host-объекты, ассемблер и `//go:cgo_import_dynamic` на объявленный каталог и его копию в `vendor`. Послабление для аудированного стороннего кода в `vendor` к заменённому модулю не применяется. Ключ кэша, идентичность рецепта и путь артефакта не меняются. Установки схемы 8 записываются маркером v4; маркеры v1, v2 и v3 сохраняют свои диапазоны версий манифеста.
- Схема 8 разбирает закрытую пару `execution_policy` и `interpreter` на скриптовой команде. csk не реализует воркер `script-worker-v1`, поэтому установка такой команды отклоняется закрытой диагностикой `script_execution_policy_unsupported` вместо публикации обычного шима: и предварительная проверка установки, и запись шима отказывают самостоятельно.
