...
Коммитьте `Skillfile.lock.json` в репозиторий проекта вместе со `Skillfile.json`, как `package-lock.json`. Лок фиксирует разрешённые источники и их содержимое, а `csk install` по локу берёт байты только из хранилища снимков в домашнем каталоге csk. Поэтому на машине без этих снимков `csk install` по закоммиченному локу завершается отказом `source_snapshot_unavailable` для источников любого вида. Восстанавливает установку `csk upgrade`: команда заново разрешает теги и ветки и перечитывает каталоги `path`. При тех же commit и неизменном содержимом `path` новый лок совпадает с закоммиченным побайтово. Приватные привязки машины в лок не попадают, коммитить их не нужно.
...
- [`docs/authoring-cli-commands.md`](docs/authoring-cli-commands.md): руководство по поставке CLI-утилит в скиллах (Go, скрипт-команды, ограничения компилятора и планируемые языки).
- [`docs/troubleshooting.md`](docs/troubleshooting.md): диагностика ошибок установки, миграция со старых версий и решение частых проблем.
