...
- `csk.sources.selection`: `resolve_selector_directory` (`.` selects the
  ...
  within and across selectors and against caller `reserved_names`).
...
- Bounds: snapshot/staging trees live under the csk home (`source-v1`
  ...
  transitive dependency-identity conflicts beyond direct installed names
  belong to the closure story; the frontmatter parser rejects YAML outside
