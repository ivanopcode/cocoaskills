...
- Equal-version snapshots must retain the same head, Merkle root, and log size.
  The high-water state is keyed by canonical registry URL and survives signing
  key rotation. It lives in protected durable state outside the disposable
  response cache. Existing corruption and persistence failure are fail-closed.
...
## 15. Curator Protocol 1.0 production profile

The shared protocol release makes the following previously implicit client
behavior normative without changing any deployed JSON object or filename:

- identity, commit, and content-hash query filters are conjunctive;
- pagination keeps one snapshot-bound cursor chain and rejects cursor cycles,
  cursors over 4,096 characters, more than 10,000 records, and responses over
  16 MiB;
- GET retries are limited to network errors, `429`, and `503`; publication
  retries require the identical record bytes and deterministic
  `Idempotency-Key`; every loop has three total attempts and a finite total
  deadline;
- redirects are rejected, so queries do not change and bearer tokens do not
  move to another endpoint;
- rollback state is stored under the machine `state/registry` directory,
  migrated from the legacy response-cache location, written atomically before
  acceptance, and included in protected machine backups.

These behaviors are executed from the authoritative `registry-client.json`
