...
- `skill-youtrack/scripts/yt` and `ytx` call adjacent Python modules and
  helper scripts.
- `skill-gitlab/scripts/gmr` calls `gmr_main.py` and shared helpers.
- `skill-sentry/scripts/sentry-api` and `sentry-cli-auth` call adjacent
...
```json
{
  ...
  "runtime_roots": ["scripts"],
  "commands": {
    "gmr": {"type": "script", "unix_path": "scripts/gmr"}
  }
}
```
...
1. Migrate `skill-youtrack` as the pilot:
   - `schema_version: 2`
   - `runtime_roots: ["scripts"]`
   - commands: `yt`, `ytx`
2. Migrate `skill-gitlab`:
   - `schema_version: 2`
   - `runtime_roots: ["scripts"]`
   - script command: `gmr`
   - system command: `glab`
3. Migrate `skill-sentry`:
   - `schema_version: 2`
   - `runtime_roots: ["scripts"]`
   - script commands: `sentry-api`, `sentry-cli-auth`
   - system command: `sentry-cli`
4. Tag each skill repository with a new release tag.
5. Update `partners-app` `Skillfile.json` to those tags.
6. Only after observation, remove legacy `agents/runtime.json` and
   `dependencies.json` from the skill repositories.

Partners-app integration details such as `.mise.toml`, `make agents`, stamp
...
7. Update `partners-app` to consume the new skill tags.
