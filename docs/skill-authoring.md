...
### Чего пакет скилла не делает

- **Не коммитьте собранные бинарники** ни в скилл, ни в билд-репозиторий: они обходят trust-модель целиком (нет идентичности тулчейна, receipt, защищённого кеша). Тип команды `binary` не существует, манифест с ним не установится.
- **Не пишите в runtime-дерево установленного скилла.** Скрипт, который бутстрапит `.venv` рядом с собой в `~/.cocoaskills/runtime/...`, оставляет симлинки в защищённом дереве, и следующая установка падает с `unsafe transaction tree entry`. Кеши и окружения держите в директориях пользователя (XDG, `~/.cache`), не в runtime.
- **Не держите `agents/runtime.json`.** Это легаси-фоллбэк: старые csk читают его вместо манифеста и превращают "нужен csk новее" в бессмысленный `source file not found`.
...
### Схема v8: корни первопартийных модулей и политика исполнения скриптов

Схема v8 добавляет ровно два поля. Команда `go-v1` получает необязательный
список `modules`: каталоги первопартийных модулей Go того же пакета, которые
корень сборки заменяет через `replace`. Скриптовая команда получает пару
`execution_policy` и `interpreter`.
...
Пакет заявляет список, менеджер его проверяет. Замена в `go.mod` никогда не
читается как инструкция: csk сверяет объявление с материализованным набором
директив из `<build_root>/vendor/modules.txt` один к одному. Директива без
объявления отклоняется как `build_module_root_directive_undeclared`, объявление
без директивы как `build_module_root_declaration_unused`, форма вне
директорийной как `build_module_root_directive_form_unsupported`, нарушение
размещения как `build_module_root_containment_invalid`. Отсутствующий или
пустой список сохраняет смысл схем 6 и 7: набор замен обязан быть пустым.

Правила корней модулей:

- Каждый каталог представляет собой портируемый относительный путь, отличный
  от `.`, к реальному каталогу без ссылок строго внутри снапшота скилла.
- Каталог содержит `go.mod` напрямую.
- Каталоги уникальны, попарно не пересекаются и не совпадают, не содержат и не
  содержатся в любом корне сборки или корне runtime. Сравнение выполняется и
  после платформенного отображения путей, поэтому два каталога, различающиеся
  только регистром, отклоняются и на регистрозависимой файловой системе.
- Проверка объявления и размещения завершается до фиксированного `go list`,
  проверка формы и биекции - после его возврата и до `go build`.
- Под `-mod=vendor` компилятор читает копию модуля из `vendor`; csk не сверяет
  две копии и не считает расхождение ошибкой. Обе копии лежат внутри
  проверенного снапшота и уже входят в идентичность исходников сборки.
- Скан на cgo, host-объекты, ассемблер и директиву `//go:cgo_import_dynamic`
  распространяется на объявленный каталог и на его копию в `vendor`. Послабление
  для аудированного стороннего кода в `vendor` к заменённому модулю не
  применяется.
- Ключ кэша, идентичность рецепта, маркер установки и путь артефакта не
  меняются.

Скриптовая команда схемы v8 может выбрать закрытую политику исполнения:
...
Поля `execution_policy` и `interpreter` совместно обязательны; допустимые
значения - `script-worker-v1` и `python3-v1` либо `node-v1`. Отсутствие пары
означает declared-only на любой схеме.

csk разбирает эту пару, но не реализует воркер `script-worker-v1`. Установка
такой команды отклоняется закрытой диагностикой
`script_execution_policy_unsupported`: менеджеру без воркера запрещено
устанавливать команду declared-only, понижать её или игнорировать поле, потому
что опубликованный шим запустил бы код пакета с полными правами вызывающего.
Скиллы схемы v8 с declared-only скриптами и с корнями модулей
...
- Поле `modules` команды `go-v1` требует схему 8. Значение по умолчанию: `[]`.
...
Шаблон резолва команд в `SKILL.md` (гасит предупреждение `skill.command_resolution_contract_missing`):
...
1. Проверьте рабочую копию командой `csk skill check .` (и `csk skill check . --locale <locale>`): `csk skill check` завершается кодом 0 и при предупреждениях, поэтому проверяйте вывод на отсутствие ошибок и предупреждений, а не код возврата.
