...
активируйте shell-hook для текущей сессии:

```bash
eval "$(csk shell-init zsh)"
```

Для постоянной настройки не запускайте Python CLI на старте каждой новой
вкладки. Один раз атомарно закэшируйте hook и добавьте напечатанную csk команду
`source` в `.zshrc`, `.bashrc` или профиль PowerShell:

```bash
csk shell-init zsh --install
```

После обновления CocoaSkills повторите команду с `--install`, чтобы кэш получил
исправления. Переменная `CSK_AUTO_ENV=0`, заданная до загрузки hook, отключает
обход проектных директорий на зависшем или нездоровом filesystem. Глобальные
команды останутся активны, а проектные тогда нужно вызывать явно из
`.agents/bin`.
...
| `csk shell-init [zsh\|bash\|powershell]` | Напечатать shell-hook для глобальной и проектной авто-активации `PATH`. `--install` атомарно кэширует его и печатает команду загрузки для профиля; `--no-global` ограничивает активацию чекаутами проектов. |
