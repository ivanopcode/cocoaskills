...
### Выборочные глобальные операции

`csk global install`, `csk global update` и `csk global upgrade` по умолчанию
работают со всеми объявленными глобальными скиллами. `--only <name>` сужает
запуск до одной декларации; повторите флаг, чтобы выбрать несколько:
...
Выбранный скилл всё равно втягивает свои required-скиллы в замыкание, поэтому
зависимость ставится, даже если её не называли. Всё остальное остаётся ровно
как было: невыбранная декларация не клонируется и не фетчится, а уже
установленный скилл вне выборки сохраняет свой контекст, shim-ы, runtime-дерево
и записи адаптеров агентов. Поэтому `--only` это способ поставить один скилл,
когда другая декларация указывает на репозиторий, недоступный с этой машины;
для приватного build-репозитория сочетайте флаг с операторским SSH-интерфейсом
из [docs/external-build-repositories.md](docs/external-build-repositories.md).

Сверка состояния тоже сужается: обычный `csk global install` по-прежнему
удаляет скиллы, которые больше не объявлены, а `--only` не удаляет ничего вне
выбранного замыкания. Имя, не объявленное в глобальном Skillfile, это ошибка, а
не тихий no-op.
...
| `csk global install` | Установить все глобально объявленные скиллы без фетча. `--only <name>` (повторяемый) ограничивает запуск этой декларацией и её required-замыканием. |
| `csk global update` | Зафетчить исходные репозитории глобально объявленных скиллов. `--only <name>` (повторяемый) фетчит только эту декларацию. |
| `csk global upgrade` | Выполнить глобальный update, затем глобальный install. `--dry-run` пропускает update и строит неперсистентный план установки. `--only <name>` (повторяемый) ограничивает обе фазы. |
